...
// PCSPTokenRewardIterator is returned from FilterReward and is used to iterate over the raw logs and unpacked data for Reward events raised by the PCSPToken contract.
type PCSPTokenRewardIterator struct {
	Event *PCSPTokenReward // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PCSPTokenRewardIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *PCSPTokenRewardIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PCSPTokenRewardIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PCSPTokenReward represents a Reward event raised by the PCSPToken contract.
type PCSPTokenReward struct {
	To        common.Address
	Amount    *big.Int
	RiskScore *big.Int
	Raw       types.Log // Blockchain specific contextual infos
...
// FilterReward is a free log retrieval operation binding the contract event 0x02a6a2be713fedf52f113c0a759f1c1a23a113476d9b1b1a2a453c910660de4e.
//
// Solidity: event Reward(address to, uint256 amount, uint256 riskScore)
func (_PCSPToken *PCSPTokenFilterer) FilterReward(opts *bind.FilterOpts) (*PCSPTokenRewardIterator, error) {

	logs, sub, err := _PCSPToken.contract.FilterLogs(opts, "Reward")
	if err != nil {
		return nil, err
	}
	return &PCSPTokenRewardIterator{contract: _PCSPToken.contract, event: "Reward", logs: logs, sub: sub}, nil
}

// WatchReward is a free log subscription operation binding the contract event 0x02a6a2be713fedf52f113c0a759f1c1a23a113476d9b1b1a2a453c910660de4e.
//
// Solidity: event Reward(address to, uint256 amount, uint256 riskScore)
func (_PCSPToken *PCSPTokenFilterer) WatchReward(opts *bind.WatchOpts, sink chan<- *PCSPTokenReward) (event.Subscription, error) {

	logs, sub, err := _PCSPToken.contract.WatchLogs(opts, "Reward")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PCSPTokenReward)
				if err := _PCSPToken.contract.UnpackLog(event, "Reward", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseReward is a log parse operation binding the contract event 0x02a6a2be713fedf52f113c0a759f1c1a23a113476d9b1b1a2a453c910660de4e.
//
// Solidity: event Reward(address to, uint256 amount, uint256 riskScore)
func (_PCSPToken *PCSPTokenFilterer) ParseReward(log types.Log) (*PCSPTokenReward, error) {
	event := new(PCSPTokenReward)
	if err := _PCSPToken.contract.UnpackLog(event, "Reward", log); err != nil {
		return nil, err
	}
